(function () {
    ...
    app.controller('rsspushAnalyticViewController', ['$scope', 'campaignAnalyticSrvc', '$uibModal', '$state', '$cookies', function ($scope, campaignAnalyticSrvc, $uibModal, $state, $cookies) {
        ...
        vm.campaignName = "";
        var getDate = [];
        vm.columnForOrdering = 'date';
        vm.requiredOrder = 'desc'
        vm.sortOrder = false;
        vm.colNameArray = ['date', 'rssName', 'rssURL', 'sent', 'click'];
        vm.handler = {
        ...
        vm.loadCampaigns = function (limit, pageIndex) {
            ...
            var data;
            campaignAnalyticSrvc.getRSSCampaignsAnalytics(filter).then(function (response) {
                    ...
                    if (vm.campaigns) {
                        vm.campaigns = vm.campaigns.map(function (item) {
                            if (item.sent === 0) {
                                ...
                                item.cr = ((item.click / item.sent) * 100).toFixed(2);
                            }
