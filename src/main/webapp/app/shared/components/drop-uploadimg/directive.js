(function () {
    ...
    app.directive('dropUploadimg', function ($timeout, fileReader, $uibModal) {
        return {
            restrict: 'E',
            scope: {
                image: '=',
            ...
            templateUrl: 'app/shared/components/drop-uploadimg/template.html',
            link: function (scope, element, attribute) {
                ...
                function Init() {
                    ...
                    var fileselect = $id("fileselect5"),
                        filedrag = $id("filedrag5"),
                ...
                function FileSelectHandler(e) {
                    ...
                    if (files[0]) {
                        ...
                        if (validFormats.indexOf(ext) === -1) {
                            ...
                            scope.messageBox("image size should be equal or less than 500 KB.");
                            return;
                    ...
                    $timeout(function () {
                        if (files.length > 0) {
                            ...
                            fileReader.readAsDataUrl(files[0], scope).then(function (result1) {
                                ...
                                var i = new Image();
                                i.onload = function () {
                                    if (i.width > 360 || i.height > 240) {
                                        $('#fileselect5').val('');
                                        scope.messageBox("Image dimensions should not be more than 360*240 pixel.");
                                        scope.image = "";
                                        // image.imageSrc="";
                                        return;
                                    }
                                        ...
                                        fileReader.readAsDataUrl(scope.images[scope.images.length - 1], scope).then(function (result) {
                                            ...
                                            scope.images[scope.images.length - 1].imgtype = 'custom';
                                            scope.selectImage(scope.images[scope.images.length - 1]);
                                            $id("filedrag5").style.height = "426px";
                                            //$id("outerDiv").className += " mrg-bot-20";
                                        });
                                    }
                                };
                ...
                scope.removeImage = function (image) {
                    ...
                    $id("outerDiv1").className = "col-sm-12";
                    $id("filedrag5").className = "border-gray-dashed pdd-top-bot-30  mrg-bot-30 text-center";
                    $id("filedrag5").style = "position: relative; display: block;";
